...
{
	return i2c_read(TPS65216_CHIP_PM, src_reg, 1, src_val, 1);
}
		       ...
		       uchar mask)
{
	 ...
	 * mask
	 */
	if (mask != TPS65216_MASK_ALL_BITS) {
		ret = i2c_read(TPS65216_CHIP_PM, dest_reg, 1, &read_val, 1);
		if (ret)
			return ret;
		read_val &= (~mask);
	...
	if (prot_level > 0) {
		xor_reg = dest_reg ^ TPS65216_PASSWORD_UNLOCK;
		ret = i2c_write(TPS65216_CHIP_PM, TPS65216_PASSWORD, 1,
				&xor_reg, 1);
		if (ret)
			return ret;
	}

	ret = i2c_write(TPS65216_CHIP_PM, dest_reg, 1, &dest_val, 1);
	if (ret)
		return ret;
...
{
	if ((dc_cntrl_reg != TPS65216_DEFDCDC1) &&
	    (dc_cntrl_reg != TPS65216_DEFDCDC2) &&
	    (dc_cntrl_reg != TPS65216_DEFDCDC3) &&
			(dc_cntrl_reg != TPS65216_DEFDCDC4))
		return 1;

	/* set voltage level */
	if (tps65216_reg_write(TPS65216_PROT_LEVEL_1, dc_cntrl_reg, volt_sel,
			       TPS65216_MASK_ALL_BITS))
		return 1;

	/* set GO bit to initiate voltage transition */
	if (tps65216_reg_write(TPS65216_PROT_LEVEL_1, TPS65216_DEFSLEW,
			       TPS65216_DCDC_GO, TPS65216_DCDC_GO))
		return 1;
